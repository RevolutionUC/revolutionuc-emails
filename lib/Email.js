...
class Email {
  ...
  build(templateName, templateData, from, to) {
    return this.getTemplate(templateName)
        .then(data => {
          ...
          templateData.dirname = `${__dirname}/../templates/`
   ...
   * @returns {Promise}
   */
  compileSass(htmlSource, stylesheetPath = `${__dirname}/../templates/scss/main.scss`) {
    return new Promise((resolve, reject) => {
      const html = cheerio.load(htmlSource, { decodeEntities: false })

      let nodeModulesPath = (__dirname.includes('node_modules/revolutionuc-emails')) ? path.resolve(__dirname, '../../') : path.resolve(__dirname, '../node_modules')

      sass.render({ file: stylesheetPath, includePaths: [nodeModulesPath] }, (error, result) => {
        if (error) {
          reject(error)
          return
        }
  ...
  inlineCss(htmlSource) {
    ...
    return juice(htmlSource, { applyWidthAttributes: false })
  }
